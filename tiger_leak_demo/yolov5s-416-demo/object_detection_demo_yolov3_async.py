...
def scale_bbox(x, y, height, width, class_id, confidence, im_h, im_w, is_proportional):
    ...
    x = int(x*im_w/640)
    y = int(y*im_h/640)
    w = int(width*im_w/640)
    h = int(height*im_h/640)
    '''
    
    x = int(x*im_w/416)
    y = int(y*im_h/416)
    w = int(width*im_w/416)
    h = int(height*im_h/416)
    '''
    x = int(x*im_w/208)
    y = int(y*im_h/208)
    w = int(width*im_w/208)
    h = int(height*im_h/208)
    '''
...
def parse_yolo_region(predictions, resized_image_shape, original_im_shape, params, threshold, is_proportional):
    ...
    side_layer_map = {80:0, 40:1, 20:2}
    side_layer_map = {52:0, 26:1, 13:2}
    
    _1, _2, out_blob_h, out_blob_w = predictions.shape
    '''
    _, k, ch, out_blob_h, out_blob_w = predictions.shape
    if(ch != 85 or k != 3):
        return []
    ##set_printoptions(threshold=20*20*85*3)
    print('predictions:', predictions.shape)
    print(predictions)
    '''
    for ii in range(_1):
        for jj in range(_2):
            for pp in range(out_blob_h):
                for qq in range(out_blob_w):
                    #print('predictions[ii][jj][pp][qq]---->',predictions[ii][jj][pp][qq])
                    predictions[ii][jj][pp][qq] = 1.0/(1.0+exp(-1*predictions[ii][jj][pp][qq]))
    #reshaped = predictions.reshape((_1,3,85 ,out_blob_h, out_blob_w))
    ...
    bbox_size = params.coords + 1 + int(_2/params.num -5)###params.classes
    # ------------------------------------------- Parsing YOLO Region output -------------------------------------------
    for row, col, n in np.ndindex(params.side, params.side, params.num):
    ###for row, col, n , c in np.ndindex(out_blob_h, out_blob_w, k, ch):
        # Getting raw values for each detection bounding box
        #print('params.num', params.num)
        bbox = predictions[0, n*bbox_size:(n+1)*bbox_size, row, col]
        ####print(n,row,col)
        ##bbox = predictions[0, n, :, row, col]
        #print('bbox.shape:', bbox.shape)
        
        x, y, width, height, object_probability = bbox[:5]
        ##print('wh-------->',width,height)
        ###print('object_probability:',object_probability)
        ...
        width = (2*width)**2* params.anchors[side_layer_map[out_blob_w]][2 * n]
        height = (2*height)**2 * params.anchors[side_layer_map[out_blob_h]][2 * n + 1]
        ##print('width:',width,' height:',height)
        class_id = np.argmax(class_probabilities)

        confidence = object_probability ###yyp added
        '''
...
def get_objects(output, net, new_frame_height_width, source_height_width, prob_threshold, is_proportional):
    objects = list()
    dic_layers = {'Transpose_273':(1,255,80,80),'Transpose_289':(1,255,40,40),'Transpose_305':(1,255,20,20)}
    dic_layers = {'Conv_59':(1,128,80,80),'Conv_83':(1,128,80,80),'Conv_65':(1,64,80,80),'Conv_43':(1,64,160,160),'Conv_78':(1,64,80,80), 'Conv_113':(1,256,20,20),'Conv_134':(1,256,20,20),'Conv_224':(1,128,80,80),'Conv_258/YoloRegion':(1,255,80,80),'Conv_274/YoloRegion':(1,255,40,40),'Conv_290/YoloRegion':(1,255,20,20),'Conv_41':(1,32,320,320),'Conv_258':(1,255,80,80),'Conv_274':(1,255,40,40),'Conv_290':(1,255,20,20),
'Conv_455':(1,36,80,80),'Conv_471':(1,36,40,40),'Conv_487':(1,36,20,20)}
    dic_layers = {'Conv_455':(1,36,52,52),'Conv_471':(1,36,26,26),'Conv_487':(1,36,13,13),'Conv_258':(1,255,52,52),'Conv_274':(1,255,26,26),'Conv_290':(1,255,13,13),'Conv_200':(1,255,52,52),'Conv_216':(1,255,26,26),'Conv_232':(1,255,13,13),'Conv_159':(1,36,52,52),'Conv_175':(1,36,26,26),'Conv_191':(1,36,13,13)}
    for layer_name, out_blob in output.items():
        ###out_blob = out_blob.buffer.reshape(net.layers[net.layers[layer_name].parents[0]].out_data[0].shape)
        print(layer_name)
        #continue
        ##########out_blob = out_blob.buffer.reshape(dic_layers[layer_name])
        print(out_blob.shape)
        print(out_blob)
        #continue
        print('layer_params:', net.layers[layer_name].params)
