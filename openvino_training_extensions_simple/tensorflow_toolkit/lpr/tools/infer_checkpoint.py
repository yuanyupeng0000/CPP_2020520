...
def infer(config):
  ...
  with graph.as_default():
    with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=False):
      ...
      print('\n inference output prob :\n')
      print(prob)
      prob = tf.transpose(prob, (1, 0, 2))  # prepare for CTC  ###[88, ?, 71] ?:batch_size
      print(prob)
      print('tf.shape:{0}'.format(tf.shape(prob)[0]))
      ...
      print('\n ctc_greedy_decoder output:\n')
      print(result)

      predictions = tf.to_int32(result[0][0])
      print('\n ctc_greedy_decoder predictions:\n')
      print(predictions)
