...
def inference(rnn_cells_num, input, num_classes):
  ...
  with slim.arg_scope([slim.conv2d, slim.fully_connected],
    ...
    print('\n classes:')
    print(classes)
    flattened_classes = slim.flatten(classes)
    print('\n flattened:')
    print(flattened_classes)
    pattern = slim.fully_connected(flattened_classes, rnn_cells_num)  # patterns number
    print('\n pattern:')
    print(pattern)
  ...
  '''
  LPRnet raw output:
  Tensor("Dropout_1/dropout/mul:0", shape=(32, 1, 88, 256), dtype=float32)

  classes:
  Tensor("Conv_14/Relu:0", shape=(32, 1, 88, 71), dtype=float32)

  pattern:
  Tensor("fully_connected/Relu:0", shape=(32, 128), dtype=float32)

  88

  pattern:
  Tensor("Reshape:0", shape=(32, 1, 1, 128), dtype=float32)

  pattern after tile:
  Tensor("Tile_1:0", shape=(32, 1, 88, 128), dtype=float32)

  inf:
  Tensor("concat:0", shape=(32, 1, 88, 199), dtype=float32)

  inf after conv2d:
  Tensor("Conv_15/BiasAdd:0", shape=(32, 1, 88, 71), dtype=float32)

  inf after squeeze:
  Tensor("Squeeze:0", shape=(32, 88, 71), dtype=float32)
  '''
