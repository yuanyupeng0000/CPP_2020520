...
def resnet_v1_10_1(features,
  ...
  predictions = {
    ...
    "colors": backbone[1]['predictions_color'],
    "types_class": tf.map_fn(type_to_one_hot, backbone[1]['predictions_types'], dtype=tf.int32),
    "types_color": tf.map_fn(type_to_one_hot, backbone[1]['predictions_color'], dtype=tf.int32)
  }
  ...
  loss_color = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.argmax(labels[:, 4:11], 1), 
                                                                             logits=predictions['colors']))
  ...
  loss = tf.cond(tf.train.get_global_step() < 100000,
                 lambda: loss_color + 0.001 *loss_type,
  ...
  eval_metric_ops = {
    ...
    "Type accuracy - color average": tf.metrics.accuracy(labels=labels[:, 4:11], predictions=predictions['types_color']),
    "Type accuracy - class average": tf.metrics.accuracy(labels=labels[:, 0:4], predictions=predictions['types_class']),
    "Type accuracy - XJ": tf.metrics.accuracy(labels=labels[:, 0], predictions=predictions['types_class'][:, 0]),
    "Type accuracy - KE_MB": tf.metrics.accuracy(labels=labels[:, 1], predictions=predictions['types_class'][:, 1]),
    "type accuracy - SUV": tf.metrics.accuracy(labels=labels[:, 2], predictions=predictions['types_class'][:, 2]),
    "Type accuracy - HO": tf.metrics.accuracy(labels=labels[:, 3], predictions=predictions['types_class'][:, 3]),
    "Type accuracy - Orange-Red": tf.metrics.accuracy(labels=labels[:, 4], predictions=predictions['types_color'][:, 0]),
    "Type accuracy - Yellow": tf.metrics.accuracy(labels=labels[:, 5], predictions=predictions['types_color'][:, 1]),
    "type accuracy - Blue": tf.metrics.accuracy(labels=labels[:, 6], predictions=predictions['types_color'][:, 2]),
    "Type accuracy - Silvery": tf.metrics.accuracy(labels=labels[:, 7], predictions=predictions['types_color'][:, 3]),
    "Type accuracy - White": tf.metrics.accuracy(labels=labels[:, 8], predictions=predictions['types_color'][:, 4]),
    "Type accuracy - Black": tf.metrics.accuracy(labels=labels[:, 9], predictions=predictions['types_color'][:, 5]),
    "type accuracy - Mix-Color": tf.metrics.accuracy(labels=labels[:, 10], predictions=predictions['types_color'][:, 6])
  }
  '''
  eval_metric_ops_bak = {
