...
class BarrierAttributesJson:
  ...
  @staticmethod
  def color_annotation_to_one_hot(item):
    colotype = np.zeros(7)
    if item in ('red', 'orange', 'brown', 'purple'):
      colotype[0] = 1 #red-orange-brown
    elif item in ('yellow','golden'):
      colotype[1] = 1 #yellow
    elif item in ('green', 'blue', 'cyan'):
      colotype[2] = 1 #blue
    elif item in ('silvery', 'gray'):
      colotype[3] = 1 #silvery
    elif item == 'white':
      colotype[4] = 1 #whilte
    elif item == 'black':
      colotype[5] = 1 #
  ...
  @staticmethod
  def one_hot_annotation_to_color(t):
    if t[0] == 1:
      colortype = 'red'
    elif t[1] == 1:
      colortype = 'yellow'
  ...
  def _get_annotation(frame):
    ...
    for item in frame['objects']:
      if item['label'] == 'vehicle' and \
        ...
        '''
        veh_color = BarrierAttributesJson.color_annotation_to_one_hot(item['attributes']['color'])
        ...
        anno = np.concatenate((veh_type, veh_color))
