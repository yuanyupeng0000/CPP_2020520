  ...
  with open(config.infer.annotation_path) as f:
    ...
    for _ in range(len(data)):
      ...
      for i in predict:
        ...
        print(colorcar[0][0][0], colorcar[0][0][1], colorcar[0][0][2])
...
def infer(config):
  ...
  with open(config.infer.annotation_path) as f:
    ...
    for _ in range(len(data)):
      ...
      it = 0
      summ_temp = 0
      img_name = data[pic]['image'].split('/')[-1]
      print(img_name)
      for i in predict:
        n = cars[it]
        
        bbox_car = data[pic]['objects'][n]['bbox']
        cv2.rectangle(img, (int(bbox_car[0]), int(bbox_car[1])), (int(bbox_car[2]), int(bbox_car[3])), (230,0,255), thickness=1)        
        colotype = BarrierAttributesJson.one_hot_annotation_to_color(i['types_color'])
        gtcolor = data[pic]['objects'][n]['attributes']['color']
        
        vtype = BarrierAttributesJson.one_hot_annotation_to_type(i['types_class'])
        gttype = data[pic]['objects'][n]['attributes']['type']
        confidence = 0.6
        
        if gtcolor in ('red', 'orange', 'brown', 'purple'):
          gtcolor = 'red'
        if gtcolor in ('green', 'blue', 'cyan'):
          gtcolor = 'blue'
        if gtcolor in ('yellow','golden'):
          gtcolor = 'yellow'
        if gtcolor in ('silvery', 'gray'):
          gtcolor = 'silvery'
        
        line_gt = '{}\t{}\t{}\t{}\t{}\n'.format(img_name, 'labels:', gtcolor, confidence, bbox_car)
        line_te = '{}\t{}\t{}\t{}\t{}\n'.format(img_name, 'Out:', colotype, confidence, bbox_car)
        f_te.write(line_te)
        f_gt.write(line_gt)
        #line_vg = '{}\t{}\t{}\t{}\t{}\n'.format(img_name, 'labels:', gttype.upper(), confidence, bbox_car)
        #line_vt = '{}\t{}\t{}\t{}\t{}\n'.format(img_name, 'Out:', vtype, confidence, bbox_car)
        #f_vt.write(line_vt)
        #f_vg.write(line_vg)
        ...
        ##cv2.putText(img, vtype + " " + colotype, (35, 15 + 60 * it), font, 1, (0, 0, 0), 2, cv2.LINE_AA)
        ##cv2.putText(img, gttype + " " + gtcolor, (35, 35 + 60 * it), font, 1, (0, 0, 0), 2, cv2.LINE_AA)
        ##cv2.putText(img, vtype.lower(), (int(bbox_car[0]), int(bbox_car[1])+10), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        ##cv2.putText(img, colotype.lower(), (int(bbox_car[0]), int(bbox_car[1])+25), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        ##cv2.putText(img, gttype.lower(), (int(bbox_car[0]), int(bbox_car[1])+10), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        ##cv2.putText(img, gtcolor.lower(), (int(bbox_car[0]), int(bbox_car[1])+25), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        it += 1
        ###summ_temp += l2diss
      summ_temp /= it
      summ += summ_temp
      pic += 1
      ##cv2.imshow("example", img)
      ###cv2.imwrite('anno_png/' + img_name, img)
      ##press = cv2.waitKey(0)
      ##if press == 27:
      ##  break
  summ /= len(data)
